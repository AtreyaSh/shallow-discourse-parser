...
def combination(network1, network2, iterations = 20):    
    ...
    for network in [network1, network2]:
        ...
        f = open(os.path.dirname(network)+"/inoutTrain.pickle", "rb")
        input_train, output_train = pickle.load(f)
        f.close()
        f = open(os.path.dirname(network)+"/inoutDev.pickle", "rb")
        input_dev, output_dev = pickle.load(f)
        f.close()
        f = open(os.path.dirname(network)+"/inoutTest.pickle", "rb")
