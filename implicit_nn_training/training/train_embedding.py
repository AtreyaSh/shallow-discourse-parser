...
def start_vectors(parses_train_filepath, parses_dev_filepath, parses_test_filepath, relations_train_filepath,
    ...
    if name == "m_2":
        ...
        (input_train, output_train) = convert_relations_modified_m_opt(relations_train, label_subst, m)
        (input_dev, output_dev) = convert_relations_modified_m_opt(relations_dev, label_subst, m)
        (input_test, output_test) = convert_relations_modified_m_opt(relations_test, label_subst, m)
    else:
...
def convert_relations_modified_m_opt(relations, label_subst, m):
    inputs = []
    outputs = []
    # Convert relations: word vectors from segment tokens, aggregate to fix-form vector per segment
    for i, rel in enumerate(relations):
